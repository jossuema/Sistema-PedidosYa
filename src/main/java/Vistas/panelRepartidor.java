...
import Controladores.cRepartidor;
...
public class panelRepartidor extends javax.swing.JPanel {
    ...
    //datos globales
    public cRepartidor list = new cRepartidor();
    ...
    public Repartidor leer() {
        Repartidor ob = null;
        if (form_validado()) {
            ob = new Repartidor();
            ob.Cedula_repartidor = txtCedula.getText();
    ...
    public final void ver_registro(int pos) {
        if (pos >= 0 && pos < list.Count()) {
            Repartidor ob = list.getRepartidor(pos);
            txtCedula.setText(ob.Cedula_repartidor);
    ...
    public void buscar_varios() {
        try {
            cRepartidor p = list.buscar_CedulaRepartidor(txtDato.getText()); //busca por cedula
            if (p.Count() == 0) {
                p = list.buscar_Nombre(txtDato.getText()); //buscar por apellido
            }
     ...
     */
    public panelRepartidor(String fun) {
        this.funcion =  fun;
        initComponents();
        try {
            list.leer();
        } catch (IOException ex) {
            Logger.getLogger(panelRepartidor.class.getName()).log(Level.SEVERE, null, ex);
        }
		//formato de la tabla
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Cédula repartidor:");
        ...
        jButton1.setText("Seleccionar Repartidor");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        int pos = tabla.getSelectedRow();
        if(pos>=0&&pos<list.Count()){
            Repartidor cl = list.getRepartidor(pos);
            frmPrincipal.repartidor = cl;

            frmPrincipal.mostrarMain("Repartidor");
        }else{
